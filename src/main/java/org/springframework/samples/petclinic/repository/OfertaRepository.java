...
public interface OfertaRepository  extends CrudRepository<Oferta, Integer> {
	...
	@Modifying
	  @Query(value = "INSERT INTO OFERTA_PIZZA(OFERTA_ID, PIZZA_EN_OFERTA_ID) VALUES (?1, ?2)",	nativeQuery = true)
		void asociarOfertaAPizza(int ofertaId, int pizzaEnOfertaId);
	
	@Modifying
	  @Query(value = "INSERT INTO OFERTA_BEBIDA(OFERTA_ID, BEBIDA_EN_OFERTA_ID) VALUES (?1, ?2)",	nativeQuery = true)
		void asociarOfertaABebida(int ofertaId, int bebidaEnOfertaId);
	
	@Modifying
	  @Query(value = "INSERT INTO OFERTA_OTRO(OFERTA_ID, OTRO_EN_OFERTA_ID) VALUES (?1, ?2)",	nativeQuery = true)
